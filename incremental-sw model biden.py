...
tfidf_set = TfidfVectorizer(analyzer='word', strip_accents = 'unicode', tokenizer = stemming_tokenizer, max_features = 5000, ngram_range = (1, 3), norm = 'l1')
tfidf_sliding = TfidfVectorizer(analyzer='word', strip_accents = 'unicode', tokenizer = stemming_tokenizer, max_features = 5000, ngram_range = (1, 3), norm = 'l1')
...
# static
clf_set = svm.SVC(C = 100, gamma = 0.001, kernel = 'linear', class_weight="balanced").fit(X_train_tfidf, y_train)
...
for index, peak in enumerate(biden_peaks):

    df_n = pd.read_csv("./Tweets/samples/Biden/biden peak " + peak + '.csv', sep = ",", encoding = 'latin1', error_bad_lines=False)
    df_n.dropna()
